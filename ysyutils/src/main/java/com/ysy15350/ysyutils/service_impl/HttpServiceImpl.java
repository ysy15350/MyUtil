...
public class HttpServiceImpl implements HttpService {
    ...
    public void requestPost(RequestOptions requestOptions, ApiCallBack apiCallBack) {
        ...
        Log.d(TAG, String.format("************************************************************\n\n requestPost() called ,%s\n\n%s\n\n************************************************************"
                , this.toString(), CommFun.getDateString("yyyy-MM-dd HH:mm:ss:SSS")));
        ...
        if (requestOptions != null) {
            ...
            if (bodyParams != null) {
                ...
                Log.d(TAG, "------------------------------------------------------------RequestParams start------------------------------------------------------------");

                Log.d(TAG, "url="+requestOptions.getUri());

                for (KeyValuePair param :
                        bodyParams) {
                    mRequestParams.addBodyParameter(param.getKeyStr(), param.getValueStr());
                    Log.d(TAG, String.format("key=%s,value=%s", param.getKey(), param.getValue().toString()));
                }

                Log.d(TAG, "------------------------------------------------------------RequestParams end------------------------------------------------------------");
            }
        }
    ...
    Callback.CacheCallback<String> cacheCallback = new Callback.CacheCallback<String>() {
        ...
        public boolean onCache(String result) {
            Log.d(TAG, "onCache() called with: useCache = ["+useCache+"], result = [" + result + "]");
